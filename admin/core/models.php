...
function getAllUsers($pdo) {
	$sql = "SELECT user_id, username, first_name, last_name, is_suspended 
			FROM users 
			WHERE is_admin = 0 
			ORDER BY username";
	$stmt = $pdo->prepare($sql);
...
function toggleUserStatus($pdo, $userId, $suspend) {
    $sql = "UPDATE users SET is_suspended = ? WHERE user_id = ? AND is_admin = 0";
    $stmt = $pdo->prepare($sql);
    return $stmt->execute([$suspend, $userId]);
}

function getAllDocumentsWithMessages($pdo) {
    ...
    // Then get messages for each document
    foreach ($documents as &$doc) {
        ...
        $doc['messages'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
